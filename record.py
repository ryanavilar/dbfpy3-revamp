...
class DbfRecord(object):
    ...
    def isExist(cls,dbf,header, find):
        find = str(find)
        for x in range(dbf.recordCount):
            string = cls.rawFromStream(dbf,x)
            _fd = dbf.header[header]
            record = _fd.decodeFromRecord(string)
            if(record == find):
                return True

        return False
    isExist = classmethod(isExist)

    def findOne(cls, dbf, header, find):
        find = str(find)
        for x in range(dbf.recordCount):
            string = cls.rawFromStream(dbf,x)
            _fd = dbf.header[header]
            ...
            if(record == find):
                print(record)
                return cls(dbf, x, string[0]=="*",
                [_fd.decodeFromRecord(string) for _fd in dbf.header.fields])

        return False

    findOne = classmethod(findOne)

    def processFound(cls,found,getOnly = None):
        if getOnly is None:
            return found
        else:
            obj = {}
            ...
            print(obj)
            return obj
    processFound = classmethod(processFound)

    def findAllWithGetOnly(cls, dbf,header, find, option= None, getOnly= None):
        result = []
        find = str(find)
        #print(getVariable)
        ...
        for x in range(dbf.recordCount):
            ...
            record = _fd.decodeFromRecord(string)
            if(option is None):
                if(str(record).find(find) > -1):
                    found = {}
                    for y in getOnly:
                        found[y] = dbf.header[y].decodeFromRecord(string)
                    result.append(found)
            elif(option == "startWith"):
                if(str(record).startswith(find)):
                    found = {}
                    for y in getOnly:
                        found[y] = dbf.header[y].decodeFromRecord(string)
                    result.append(found)
            elif(option == "endWith"):
                if(str(record).endswith(find)):
                    found = {}
                    for y in getOnly:
                        found[y] = dbf.header[y].decodeFromRecord(string)
                    result.append(found)
    ...
    findAllWithGetOnly = classmethod(findAllWithGetOnly)

    def findAll(cls, dbf,header, find, option= None, getOnly= None):
        if getOnly is not None:
            return cls.findAllWithGetOnly(dbf,header,find,option,getOnly)
        ...
        for x in range(dbf.recordCount):
            ...
            if(option is None):
                if(str(record).find(find) > -1):
                    found = cls(dbf, x, string[0]=="*",[_fd.decodeFromRecord(string) for _fd in dbf.header.fields])
                    result.append(found)
            elif(option == "startWith"):
                if(str(record).startswith(find)):
                    found = cls(dbf, x, string[0]=="*",[_fd.decodeFromRecord(string) for _fd in dbf.header.fields])
